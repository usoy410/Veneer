...
import { getDistroInfo, checkEww, restartEww } from "../lib/commands";
...
type Step = 'welcome' | 'dependencies' | 'install' | 'daemon' | 'finish';
...
export function Onboarding({ onComplete }: OnboardingProps) {
  ...
  useEffect(() => {
    const init = async () => {
      const d = await getDistroInfo();
      setDistro(d);
      const installed = await checkEww();
